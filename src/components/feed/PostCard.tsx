...
export function PostCard({ post, me, onDelete }: Props) {
  ...
  return (
    <article className="glass-hover p-5 space-y-4 animate-fade-in">
      {/* Header */}
        ...
        <Link href={`/profile/${post.author.id}`} className="flex items-center gap-3 group min-w-0">
          <div className="relative shrink-0">
            <Avatar
              ...
              className="ring-1 ring-white/10 transition group-hover:ring-accent/50"
            />
          </div>
          <div className="min-w-0">
            <div className="font-semibold text-[15px] tracking-tight truncate leading-tight group-hover:text-accent-glow transition">
              {post.author.display_name || post.author.username}
            </div>
            <div className="flex items-center gap-1.5 text-xs text-text-dim mt-0.5">
              <span className="text-text-muted/70">@{post.author.username}</span>
          ...
          <div className="relative shrink-0">
            ...
            {menuOpen && (
              <>
                ...
                <div className="absolute right-0 top-full mt-1.5 z-20 min-w-[9rem] glass-flat shadow-lift p-1 animate-slide-down">
                  <button
                    onClick={deletePost}
                    className="w-full flex items-center gap-2 px-3 py-2 text-sm text-danger rounded-xl hover:bg-danger-soft transition"
                  >
      ...
      {/* Image */}
      {post.image_url && (
        <div className="rounded-2xl overflow-hidden border border-border/60 bg-bg-inset -mx-1">
      ...
      {/* Actions */}
      <div className="flex items-center gap-0.5 pt-1 -mx-2 border-t border-border-soft">
        ...
        <button
          ...
          className={cn(
            "inline-flex items-center gap-2 px-3 py-1.5 mt-2 rounded-xl text-sm font-medium transition-all duration-150",
            showComments
      ...
      {/* Comments */}
      {showComments && (
        <div className="pt-2 border-t border-border-soft animate-fade-in">
